...
アンチウィルスソフト等にブロックされる場合があるようですが、その場合はアンチウィルスソフトを一時的に無効にしたり、除外設定をしてください。  
また、付属のツールがWindowsのセキュリティによってブロックされる場合がありますが、その場合は詳細をクリックして実行を許可してください。  
...
### コマンド（命令）一覧
コマンドはアーカイブに対して行う基本動作を1文字で指定します（大文字・小文字は区別されません）。主要なコマンドは以下の通りです。

| コマンド | 説明 |
| :--- | :--- |
| `a` | アーカイブにファイルを追加・作成します（Add）。 |
| `e` | アーカイブからファイルを展開します（Extract）。ディレクトリ構造は無視されます（※デフォルト動作）。 |
| `x` | ディレクトリ構造を維持してアーカイブを展開します（eXtract with paths）。 |
| `l` | アーカイブ内のファイル一覧を表示します（List）。 |
| `t` | アーカイブの整合性をチェック・テストします（Test）。 |
| `d` | アーカイブから指定したファイルを削除します（Delete）。 |
| `u` | 書庫内のファイルより新しい場合、または新規ファイルの場合にアーカイブに追加・更新します（Update）。 |
| `m` | ファイルをアーカイブに追加し、元のファイルを削除します（Move）。 |

### スイッチ（オプション）一覧
解凍や圧縮の動作を細かく制御するための各種スイッチ（`-d`, `-r`, `-x`, `-jm` など）の対応状況、無視するスイッチ、エラーとなるスイッチの対応詳細については、以下のドキュメントを参照してください。

- [コマンドラインスイッチ対応表 (Switches.md)](Switches.md)
...
2. **LHa 圧縮・展開エンジン**:
   ...
   このエンジンの利用にあたっては、LHa for UNIX の再配布条件を遵守する必要があります（詳細は `LhaForUnix/man/lha.man` を参照）。
   - 著作権表示を削除しないこと。
   - 改変した場合はその旨を明記すること。
   - バイナリのみの再配布は原則として許可されていません。
